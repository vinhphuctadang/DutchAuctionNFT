...
contract DutchAuction is ReentrancyGuard, IERC721Receiver {
    ...
    event CreateAuctionEvent(
        address creatorAddress, string matchId, 
        uint openBlock, uint expiryBlock, 
        uint blockPerEpoch,
        NFT[] nfts
    );
    ...
    function player_bid(string memory matchId, uint tokenIndex) external nonReentrant validTokenIndex(matchId, tokenIndex) {
        ...
        IERC20(USDC_ADDRESS).safeTransferFrom(playerAddress, address(this), amount);
    }
